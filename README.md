...
<div align="center">
    <!-- Last commit -->
    <img src="https://img.shields.io/github/last-commit/lectrum/workshop-strange-search.svg?longCache=true&style=flat-square" alt="Last commit"
    />
    <!-- Dependencies -->
...
1. [Скачай и установи](https://nodejs.org/en/) последнюю **`LTS-версию Node 14.15.4`**:
2. Выполни в консоли **`node -v`** и убедись, что установленная версия **`Node.js`** не ниже **`14.15.4`**;
3. Выполни в консоли **`npm -v`** и убедись, что установленная версия **`npm`** не ниже **`6.14.10`**;

> ❗️ Мы поддерживаем только последние **`LTS-версии Node.js`** (текущая LTS — **`Node 14.15.4`**). Мы не даём гарантий работы на других версиях Node. Если у тебя не работает, в первую очередь проверь версию Node.js!
...
```bash
git clone https://github.com/Lectrum/workshop-strange-search.git
```

7. Перейди в проект, выполнив команду:

```bash
cd workshop-strange-search
```
